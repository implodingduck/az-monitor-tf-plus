...
curl -vvv -X POST -H "Content-Encoding: gzip" -H "Content-Type: application/json" -H "Authorization: Bearer $ACCESS_TOKEN" --data-binary @body.gz "$DATA_COLLECTION_ENDPOINT/dataCollectionRules/$DATA_RULE_ID/streams/Custom-MyTableRawData?api-version=2021-11-01-preview"